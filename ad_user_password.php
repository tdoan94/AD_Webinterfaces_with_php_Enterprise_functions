...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['password'])) {
        ...
        if (empty($newPassword)) {
            ...
            // pwdLastSet nicht ändern → Benutzer muss nicht zwingend ändern

            if (@ldap_modify($ldap_conn, $user_dn, $mods)) {
                $success = "Passwort erfolgreich geändert.";
            } else {
                $error = "Fehler beim Ändern: " . ldap_error($ldap_conn);
            }
        }
    } elseif (isset($_POST['reset_password'])) {
        // Passwort zurücksetzen (random + Änderung erzwingen)
        $randomPassword = generateRandomPassword();
        $password_quoted = '"' . $randomPassword . '"';
        $password_utf16 = mb_convert_encoding($password_quoted, 'UTF-16LE');

        $mods = [];
        $mods['unicodePwd'] = [$password_utf16];
        // pwdLastSet auf 0 setzen = Benutzer muss Passwort bei nächster Anmeldung ändern
        $mods['pwdLastSet'] = ['0'];

        if (@ldap_modify($ldap_conn, $user_dn, $mods)) {
            $success = "Passwort wurde zurückgesetzt. Neues Passwort: <strong>" . htmlspecialchars($randomPassword) . "</strong><br>Der Benutzer muss das Passwort bei der nächsten Anmeldung ändern.";
        } else {
...
<head>
<meta charset="UTF-8" />
<title>Passwort ändern – <?= htmlspecialchars($username) ?></title>
<style>
...
<h2>Passwort ändern für: <?= htmlspecialchars($username) ?></h2>
...
<form method="POST" style="display:inline-block;">
    ...
    <button type="submit" class="reset" onclick="return confirm('Passwort wirklich zurücksetzen und Zufallspasswort setzen?');">Passwort zurücksetzen</button>
</form>
